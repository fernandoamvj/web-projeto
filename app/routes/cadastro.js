module.exports = function(app){
  app.get('/cadastroUser',function(req,res){
      res.render('home/cadastroUser',{errosValidacao:{}});
  });
  ...
  app.post('/salvaUser',function(req,res){
    ...
    if(erros){
      res.status(400).render('home/cadastroUser',{errosValidacao:erros});
      return;
    }
    ...
    objetoBanco.verificaLogin(user,function(err,results){
      ...
      if(results[0].login == user.login){//results retorna um array de objetos
        erros = [{param:'login',msg:'Login já existe!! Tente outro!',value:''}];
        res.status(400).render('home/cadastroUser',{errosValidacao:erros});
      }
    }else{
      connection.end();
